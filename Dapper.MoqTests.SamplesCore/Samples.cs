...
where Registration = @registration", new { registration = "ABC123" }, It.IsAny<IDbTransaction>(), null, null));
        ...
        [Test]
        public void SetupStoredProcedureAndVerify()
        {
            var connectionFactory = new Mock<IDbConnectionFactory>();
            var connection = new MockDbConnection();
            var repository = new SampleRepository(connectionFactory.Object, connection);
            var cars = new List<Car>() {new Car { Registration = "ABC123", Make = "Vauxhall", Model = "Astra" }};
            connectionFactory
                .Setup(f => f.OpenConnection())
                .Returns(connection);
            connection.Setup(c => c.Query<Car>("sp.test", new { test = "yes" }, null, true, null, CommandType.StoredProcedure))
                    .Returns(cars);

            var result = repository.GetCarsStoredProcedure();

            connection.Verify(c => c.Query<Car>("sp.test", new { test = "yes" }, null, true, null, CommandType.StoredProcedure));
        }
